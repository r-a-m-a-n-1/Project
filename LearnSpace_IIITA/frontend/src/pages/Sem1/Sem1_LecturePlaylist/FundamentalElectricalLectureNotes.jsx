...
const FundamentalElectricalLectureNotes = () => {
  ...
  useEffect(() => {
    const fetchNotes = async () => {
      try {
        ...
        const subject = "Fundamentals of Electrical & Electronics Engineering"; // <-- CHANGE THIS to your subject
    ...
    <div className="bg-gradient-to-br from-gray-900 via-black to-gray-900 text-white min-h-screen">
      <div className="sticky top-0 z-50 w-full bg-black bg-opacity-90 backdrop-blur-sm py-4">
        <h1 className="text-3xl font-bold text-center">
         Fundamentals of Electrical & Electronics Engineering Lecture Notes {/* Change title here too */}
        </h1>
      </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6">
                {Object.entries(types).map(([type, notesArr]) =>
                  notesArr.map((note, idx) => (
                    ...
                    >
                      <div>
                        <h3 className="text-lg font-semibold line-clamp-1">{type}</h3>
...
export default FundamentalElectricalLectureNotes;
